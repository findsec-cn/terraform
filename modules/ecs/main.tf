...
resource "alicloud_instance" "instances" {
  ...
  tags {
    role = "${lookup(var.instance_tags, "role")}"
    zone = "${lookup(var.instance_tags, "zone")}"
  }
}
...
resource "alicloud_disk" "disks" {
  ...
  category          = "${var.number_of_disks < 2 ? var.disk_categorys[0] : element(var.disk_categorys, count.index) }"
  size              = "${var.number_of_disks < 2 ? var.disk_sizes[0] : element(var.disk_sizes, count.index) }"

  tags {
    role = "${lookup(var.disk_tags, "role")}"
    zone = "${lookup(var.disk_tags, "zone")}"
  }
}
